...
1. **الأهم — استراتيجية الترحيل:** هل `Orders`/`OrderItems`/`CashShifts` تحل محل `Sales`/`SaleItems`/`Shifts` الحالية بالكامل (ترحيل بيانات + حذف الجداول القديمة)، أم يعمل النظامان بالتوازي لفترة (مثلاً فرع تجريبي على النظام الجديد بينما الباقي على القديم)؟ هذا يقرر ترتيب Sprint 4 فعليًا.
