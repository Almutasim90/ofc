...
export default function AiSettingsPage(){
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError(null);setMessage(null);setTestReply(null);try{const x=await api.put<AiSettings>('/api/ai/settings',{...form,baseUrl:form.provider==='Custom'?form.baseUrl:null});setLast4(x.apiKeyLast4);setForm(f=>({...f,apiKey:''}));setMessage(t('ai.saved'))}catch(err){setError(err instanceof ApiError?err.message:t('ai.saveError'))}finally{setBusy(false)}}
  const testConnection=async()=>{setTesting(true);setError(null);setMessage(null);setTestReply(null);try{const x=await api.post<{reply:string}>('/api/ai/settings/test');setTestReply(t('ai.testSuccess',{reply:x.reply}))}catch(err){setError(err instanceof ApiError?err.message:t('ai.testError'))}finally{setTesting(false)}}
  const changeProvider=(provider:string)=>{setForm({...form,provider,model:modelSuggestions[provider][0]??''})}
  return <section><h1>{t('ai.title')}</h1><p>{t('ai.description')}</p>{message&&<p className="text-primary">{message}</p>}{testReply&&<p className="text-primary">{testReply}</p>}{error&&<p className="error-text" role="alert">{error}</p>}<form className="ui-card ui-stack" onSubmit={submit}>
