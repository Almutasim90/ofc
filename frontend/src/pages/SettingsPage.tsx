...
export default function SettingsPage() {
  ...
  return <section><h1>{t('settings.title')}</h1>{message && <p className="text-primary">{message}</p>}{error && <p className="error-text">{error}</p>}<div className="grid gap-6 lg:grid-cols-2">
    ...
    <form className="ui-card ui-stack" onSubmit={changePassword}><h2>{t('settings.changePassword')}</h2><label className="flex flex-col gap-1 text-muted">{t('settings.currentPassword')}<input type="password" required value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} /></label><label className="flex flex-col gap-1 text-muted">{t('settings.newPassword')}<input type="password" required minLength={8} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} /></label><label className="flex flex-col gap-1 text-muted">{t('settings.confirmPassword')}<input type="password" required minLength={8} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} /></label><button>{t('settings.updatePassword')}</button></form>
  </div><div className="ui-stack"><input className="w-full max-w-md" placeholder={t('settingsHub.search')} value={search} onChange={e=>setSearch(e.target.value)}/><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{cards.map(c=><Link key={c.to} to={c.to} className="ui-card ui-card-interactive ui-stack min-h-11"><h2 className="truncate">{c.title}</h2><p className="line-clamp-2">{c.summary}</p></Link>)}</div></div></section>
}
