...
export default function SettingsPage() {
  ...
  const savePreferences = async (event: FormEvent) => { event.preventDefault(); try { await updatePreferences({ preferredLanguage: language, preferredTheme: theme }); await i18n.changeLanguage(language); toast.success(t('settings.saved')) } catch { toast.error(t('settings.saveError')) } }
  ...
  const allCards=[
    ...
    {to:'/receipt-settings',title:t('nav.receiptSettings'),summary:summaries.receipt??t('settingsHub.receiptSummary'),permission:'receipt.manage',group:'operations' as const,icon:'printer' as AppIconName},
