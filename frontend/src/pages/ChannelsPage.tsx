...
export default function ChannelsPage() {
  ...
  return <section className="channels-page">
    ...
    <DataTable rows={channels} getRowKey={(channel) => channel.id} shellClassName="channel-table" getSearchText={(channel) => `${channel.nameAr} ${channel.nameEn} ${channel.code}`}
      columns={[
        { id: 'logo', header: t('channels.logo'), cell: (channel) => <div className="channel-table-logo">{channel.logoUrl ? <img src={resolveApiAssetUrl(channel.logoUrl)} alt="" /> : <span>{(i18n.language === 'ar' ? channel.nameAr : channel.nameEn).charAt(0)}</span>}</div> },
        { id: 'channel', header: t('channels.channel'), cell: (channel) => <><strong>{i18n.language === 'ar' ? channel.nameAr : channel.nameEn}</strong><small> {channel.code}</small></>, sortValue: (channel) => i18n.language === 'ar' ? channel.nameAr : channel.nameEn },
        { id: 'status', header: t('channels.status'), cell: (channel) => <span className={`channel-status ${channel.isActive ? 'is-active' : ''}`}>{channel.isActive ? t('channels.active') : t('channels.inactive')}</span>, sortValue: (channel) => channel.isActive },
        { id: 'actions', header: t('common.actions'), cell: (channel) => <div className="channel-row-actions"><button onClick={() => void editPrices(channel)}>{t('channels.pricing')}</button><button className="button-secondary" onClick={() => { setSelected(channel); setForm({code:channel.code,nameAr: channel.nameAr, nameEn: channel.nameEn, logoUrl: channel.logoUrl ?? '', isActive: channel.isActive }); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>{t('common.edit')}</button>{!channel.isInStore && <button className="button-secondary" onClick={() => toggleActive(channel)}>{channel.isActive ? t('channels.stop') : t('channels.enable')}</button>}{!channel.isInStore && <button className="button-danger" onClick={() => deleteChannel(channel.id)}>{t('common.delete')}</button>}</div> },
      ]} />
