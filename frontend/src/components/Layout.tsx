...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="admin-shell min-h-screen bg-bg text-text">
    ...
    <nav className="admin-bottom-nav">{navItems.slice(0,4).map(item=><NavLink key={item.to} to={item.to}><AppIcon className="h-5 w-5" name={item.icon} /><small>{item.label}</small></NavLink>)}<button onClick={()=>setMoreOpen(true)}><AppIcon className="h-5 w-5" name="more" /><small>{t('nav.more')}</small></button></nav>
