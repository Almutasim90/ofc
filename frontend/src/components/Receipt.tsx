  ...
  return (
    <div className="receipt-print" dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="receipt-totals">
        ...
        <div><span>{t('cashier.paymentMethod')}</span><span>{t(`cashier.${sale.paymentMethod.toLowerCase()}`)}</span></div>
        {sale.paymentMethod === 'Mixed' && <><div><span>{t('cashier.cash')}</span><Money value={sale.cashAmount} /></div><div><span>{t('cashier.card')}</span><Money value={sale.cardAmount} /></div></>}
      </div>
    </div>
  )
}
