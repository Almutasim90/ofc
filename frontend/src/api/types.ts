...
export interface RestaurantOrderDto { id:string; branchId:string; orderNumber:number; orderTypeCode:string; tableId:string|null; tableLabel:string|null; status:string; subtotal:number; discountAmount:number; grandTotal:number; createdAt:string; items:{id:string;menuItemId:string;name:string;unitPrice:number;quantity:number;lineTotal:number;notes:string|null;isCancelled:boolean;modifierOptionIds:string[];comboOptionIds:string[]}[] }
...
export interface StockCountLineDto { ingredientId:string; name:string; systemQuantity:number; countedQuantity:number; varianceQuantity:number }
export interface StockCountDto { id:string; branchId:string; warehouseId:string; createdAt:string; status:'Draft'|'Finalized'; lines:StockCountLineDto[] }
