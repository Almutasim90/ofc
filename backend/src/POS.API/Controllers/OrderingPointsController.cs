...
public class OrderingPointsController(QrOrderingService service):ControllerBase
{
 ...
 [HttpPost("{id:guid}/regenerate")]public async Task<ActionResult<object>>Regenerate(Guid id,CancellationToken ct)=>Ok(new{qrCodeToken=await service.RegenerateAsync(id,ct)});
