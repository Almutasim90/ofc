...
public class RestaurantCatalogController(RestaurantCatalogService service) : ControllerBase
{
    ...
    [HttpPut("branches/{branchId:guid}/features/{key}"), RequirePermission(PermissionKeys.ProductsManage)] public async Task<ActionResult<BranchFeatureFlagDto>> SetFeature(Guid branchId, string key, SetFeatureFlagRequest request, CancellationToken ct) => Ok(await service.SetFlagAsync(branchId, key, request.IsEnabled, ct));
