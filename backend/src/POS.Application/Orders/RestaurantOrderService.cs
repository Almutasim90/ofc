...
public class RestaurantOrderService(IAppDbContext db, ICurrentUserService currentUser)
{
    ...
    public async Task<RestaurantOrderDto> CreateAsync(CreateRestaurantOrderRequest request,CancellationToken ct=default)
    {
        ...
        var order=new RestaurantOrder{Id=Guid.NewGuid(),BranchId=request.BranchId,OrderNumber=await db.ClaimNextOrderNumberAsync(request.BranchId,ct),OrderTypeId=type.Id,TableId=request.TableId,CarPlateNumber=request.CarPlateNumber?.Trim(),CashierUserId=currentUser.UserId??throw new ValidationException("Authenticated user is required."),BusinessDate=DateOnly.FromDateTime(DateTime.UtcNow),CreatedAt=DateTime.UtcNow,DiscountAmount=request.DiscountAmount,Status=RestaurantOrderStatuses.Open};
    ...
    public Task<List<RestaurantOrderDto>> GetAsync(Guid branchId,CancellationToken ct=default)=>db.RestaurantOrders.Where(x=>x.BranchId==branchId).OrderByDescending(x=>x.CreatedAt).Take(100).Select(x=>new RestaurantOrderDto(x.Id,x.BranchId,x.OrderNumber,x.OrderType.Code,x.TableId,x.Table==null?null:x.Table.Label,x.Status,x.Subtotal,x.DiscountAmount,x.GrandTotal,x.CreatedAt,x.Items.Select(i=>new OrderLineDto(i.Id,i.MenuItemId,i.MenuItemNameSnapshot,i.UnitPriceSnapshot,i.Quantity,i.LineTotal,i.Notes,i.Modifiers.Select(m=>m.ModifierOptionId).ToList(),i.ComboSelections.Select(c=>c.SelectedMenuItemId).ToList())).ToList())).ToListAsync(ct);
    private static RestaurantOrderDto ToDto(RestaurantOrder x,string code,string? table)=>new(x.Id,x.BranchId,x.OrderNumber,code,x.TableId,table,x.Status,x.Subtotal,x.DiscountAmount,x.GrandTotal,x.CreatedAt,x.Items.Select(i=>new OrderLineDto(i.Id,i.MenuItemId,i.MenuItemNameSnapshot,i.UnitPriceSnapshot,i.Quantity,i.LineTotal,i.Notes,i.Modifiers.Select(m=>m.ModifierOptionId).ToList(),i.ComboSelections.Select(c=>c.SelectedMenuItemId).ToList())).ToList());
}
