...
public class RestaurantOrderService(IAppDbContext db, ICurrentUserService currentUser)
{
    ...
    public async Task<RestaurantOrderDto> CreateAsync(CreateRestaurantOrderRequest request,CancellationToken ct=default)
    {
        ...
        if(type.Code=="CAR_PICKUP")ValidateCarPickup(request.CarPlateNumber,await db.BranchFeatureFlags.AnyAsync(x=>x.BranchId==request.BranchId&&x.FeatureKey==BranchFeatureKeys.CarPickup&&x.IsEnabled,ct));
        ...
        var order=new RestaurantOrder{Id=Guid.NewGuid(),BranchId=request.BranchId,OrderNumber=await db.ClaimNextOrderNumberAsync(request.BranchId,ct),OrderTypeId=type.Id,TableId=request.TableId,CarPlateNumber=type.Code=="CAR_PICKUP"?request.CarPlateNumber!.Trim():null,CashierUserId=currentUser.UserId??throw new ValidationException("Authenticated user is required."),CashShiftId=await db.CashShifts.Where(x=>x.BranchId==request.BranchId&&x.Status==CashShiftStatuses.Open).Select(x=>(Guid?)x.Id).FirstOrDefaultAsync(ct),SalesChannelId=await db.SalesChannels.Where(x=>x.IsInStore).Select(x=>(Guid?)x.Id).FirstOrDefaultAsync(ct),BusinessDate=DateOnly.FromDateTime(DateTime.UtcNow),CreatedAt=DateTime.UtcNow,DiscountAmount=request.DiscountAmount,Status=RestaurantOrderStatuses.Open};
