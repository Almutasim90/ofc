...
public record CreateRestaurantOrderRequest(Guid BranchId,Guid OrderTypeId,Guid? TableId,string? CarPlateNumber,decimal DiscountAmount,List<CreateOrderLineRequest> Lines,Guid? SalesChannelId=null);
...
public record RestaurantOrderDto(Guid Id,Guid BranchId,int OrderNumber,string OrderTypeCode,Guid? TableId,string? TableLabel,string Status,decimal Subtotal,decimal DiscountAmount,decimal GrandTotal,DateTime CreatedAt,List<OrderLineDto> Items,string? CarPlateNumber=null,Guid? SalesChannelId=null,string? SalesChannelCode=null);
