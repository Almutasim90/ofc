...
{
    private static readonly HashSet<string> EditTypes = ["ItemAdded","ItemRemoved","PartialRefund","PriceOverride","Other"];
    ...
    public Task<List<OrderPaymentDto>> PaymentsAsync(Guid orderId,CancellationToken ct=default)=>db.OrderPayments.Where(x=>x.OrderId==orderId).OrderBy(x=>x.CreatedAt).Select(x=>new OrderPaymentDto(x.Id,x.OrderId,x.PaymentMethod.Code,x.Amount,x.ApprovedByUserId,x.CreatedAt)).ToListAsync(ct);
    public Task<List<OrderEditLogDto>> EditsAsync(Guid orderId,CancellationToken ct=default)=>db.OrderEditLogs.Where(x=>x.OrderId==orderId).OrderByDescending(x=>x.CreatedAt).Select(x=>new OrderEditLogDto(x.Id,x.OrderId,x.UserId,x.EditType,x.Notes,x.AmountDelta,x.CreatedAt)).ToListAsync(ct);
    public async Task<OrderPaymentDto> RecordAsync(Guid orderId,RecordOrderPaymentRequest request,CancellationToken ct=default)
    {
        ...
        if(request.Amount==remaining)order.Status=RestaurantOrderStatuses.Paid;await db.SaveChangesAsync(ct);return new(row.Id,row.OrderId,method.Code,row.Amount,row.ApprovedByUserId,row.CreatedAt);
    }
    public async Task<OrderEditLogDto> EditAsync(Guid orderId,EditClosedOrderRequest request,CancellationToken ct=default)
    {
        if(!currentUser.Permissions.Contains(PermissionKeys.ClosedOrdersEdit))throw new ForbiddenException("Closed order edit permission is required."); if(!EditTypes.Contains(request.EditType))throw new ValidationException("Invalid edit type.");
        ...
        order.GrandTotal=updated;order.Subtotal+=request.AmountDelta;var log=new OrderEditLog{Id=Guid.NewGuid(),OrderId=order.Id,UserId=userId,EditType=request.EditType,Notes=string.IsNullOrWhiteSpace(request.Notes)?null:request.Notes.Trim(),AmountDelta=request.AmountDelta,CreatedAt=DateTime.UtcNow};db.OrderEditLogs.Add(log);await db.SaveChangesAsync(ct);return new(log.Id,log.OrderId,log.UserId,log.EditType,log.Notes,log.AmountDelta,log.CreatedAt);
    }
}
