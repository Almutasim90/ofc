...
public class OrderPrintingService(IAppDbContext db, IRawPrinterClient printer)
{
    ...
    {
        var order = await db.RestaurantOrders.Include(x => x.OrderType).Include(x => x.Items).ThenInclude(x => x.MenuItem).ThenInclude(x => x.PrinterSection).ThenInclude(x => x!.PrinterConfig).FirstOrDefaultAsync(x => x.Id == orderId, ct) ?? throw new NotFoundException("Order not found.");
