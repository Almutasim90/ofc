...
public class OrderCancellationTests
{
    ...
    public async Task Cancelling_item_records_audit_and_recalculates_totals()
    {
        ...
        var service=new OrderCancellationService(db,user,new POS.Application.RestaurantInventory.RestaurantInventoryService(db,user));await service.CancelItemAsync(order.Id,order.Items.First().Id,"Customer changed mind",ct);
    ...
    public async Task Paid_order_cannot_be_cancelled()
    {
        ...
        await Assert.ThrowsAsync<ValidationException>(()=>new OrderCancellationService(db,user,new POS.Application.RestaurantInventory.RestaurantInventoryService(db,user)).CancelOrderAsync(order.Id,"Wrong order",TestContext.Current.CancellationToken));
    }
